...
class Router(app_manager.RyuApp):
    ...
    tabla_vlan = { 1: '10',
                   ...
                   4: '20'}
    port_ip_mac =dict()
    interfaces_virtuales = { '10': ('00:00:00:00:01:01','255.255.255.0','192.168.1.1'), 
                    '20': ('00:00:00:00:01:02','255.255.255.0','192.168.2.1'),
                    '30': ('00:00:00:00:01:03','255.255.255.0','192.168.3.1'),
                    '40': ('00:00:00:00:01:04','255.255.255.0','192.168.4.1')}}
    ...
    def paquete_para_enrutar(self, ev):
        ...
        if eth.ethertype==0x0800: #Si es IP
            ...
            for entradas in entradas_router :
                if self.interfaces_virtuales.get(tabla_vlan[entradas])[2]==pkt_ipv4.dst:  #Si es para el ROUTER
            ...
            if comprobacion==0:
                listacoincide=[]
                for entradas in entradas_router :
                    if  ipaddr.IPv4Address(pkt_ipv4.dst) in  ipaddr.IPv4Network(self.interfaces_virtuales.get(entradas)[2]+"/"+ self.interfaces_virtuales.get(entradas)[0]):
                        mask=IPAddress(self.interfaces_virtuales.get(entradas)[0]).bin
                ...
                if(pkt_ipv4.dst not in self.tabla_enrutamiento):
                    ...
                    for puertos in self.tabla_vlan.key() :
                        if tabla_vlan.get(puertos)==entrada:
                            self.ARPREQUESTPacket(pkt_ipv4.dst,pkt_ipv4.src,puertos,datapath)
                else: #Si tenemos la mac en cache
                    print entradas
                    print("I will flow that")
                    for puerto_salida in port_ip_mac.key():
                        if port_ip_mac.get(puerto_salida)[0]==pkt_ipv4.dst:
                            actions =[ofp_parser.OFPActionSetField(eth_dst=self.tabla_enrutamiento[pkt_ipv4.dst]),
                                        ofp_parser.OFPActionSetField(eth_src=self.interfaces_virtuales.get(tabla_vlan[entradas])[0]),
                                        ofp_parser.OFPActionDecNwTtl(),
                                        ofp_parser.OFPActionOutput(puerto_salida)]

                    match = ofp_parser.OFPMatch(ipv4_dst=pkt_ipv4.dst,eth_type=ether.ETH_TYPE_IP)
                    self.add_flow( datapath=datapath,table_id=1, priority=0, match=match, actions=actions, buffer_id=msg.buffer_id)
